...
/**
 * todo:
 * @question: 1.推文采用什么样的数据结构,为什么?2.怎么维护用户id 和推文的对应关系,通过什么数据结构? 3.用户 id 和他关注的用户列表的对应关系使用什么数据结构保存? 4.怎么取最近10条推文?
 * @author Shaobo.Qian
 * @date 2020/4/14
 */
public class lc_355 {
    static class Twitter {
       ...
       private class Tweet {
         ...
         */
        public List<Integer> getNewsFeed(int userId) {//考虑堆实现
            // 由于是全局使用的，使用之前需要清空
            maxHeap.clear();
            // 如果自己发了推文也要算上
            if (twitter.containsKey(userId)) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("newsFeed = " + newsFeed.get(0));

        twitter.follow(2, 1);
        List<Integer> newsFeed1 = twitter.getNewsFeed(2);
        System.out.println("newsFeed1 = " + newsFeed1.get(0));
